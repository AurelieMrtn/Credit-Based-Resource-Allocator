# Credit-Based-Resource-Allocator
...
This project implements a backend service that manages allocation of cloud resources based on available credits.
It was originally inspired by a trading challenge, but it has been redesigned into a generic cloud resource allocator, using:
- Spring Boot
- JPA / Hibernate
...
- It owns enough allocated resources to release them
...
- Checks available credits
  - If enough credit, deducts credits and increases allocated quantity 
  - If not, throws an exception: "Insufficient credit" with code 400
- Saves the allocation as CREATED

### Retrieve an Allocation
- Fetches an allocation by its ID
- Throws an exception: "Allocation not found" with code 404 if not found

### Cancel an Allocation
- Checks if the allocation status is CREATED
  - If not, throws an exception "Allocation cannot be cancelled" with code 400
- Fetches the cost of the resource
...
- Saves the allocation as CANCELLED
...
Both repositories (AccountCreditsRepository and AllocatedResourcesRepository) are updated atomically.
...
Whenever an allocation is created or cancelled, the system updates both the AccountCredits table and the AllocatedResources table.
